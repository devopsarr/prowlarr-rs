...
pub async fn get_log_file_update_by_filename(configuration: &configuration::Configuration, filename: &str) -> Result<serde_json::Value, Error<GetLogFileUpdateByFilenameError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_filename = filename;

    let uri_str = format!("{}/api/v1/log/file/update/{filename}", configuration.base_path, filename=crate::apis::urlencode(p_path_filename));
...
pub async fn list_log_file_update(configuration: &configuration::Configuration, ) -> Result<Vec<models::LogFileResource>, Error<ListLogFileUpdateError>> {
    ...
    let uri_str = format!("{}/api/v1/log/file/update", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
